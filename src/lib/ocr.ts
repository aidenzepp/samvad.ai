...
export async function refineTranslationWithOpenAI(
...
export async function processAndTranslateDocument(
  ...
  if (useGoogleFirst) {
    const googleTranslation = await translateTextWithGoogle(
      originalText,
      targetLang
    );
    translatedText = await refineTranslationWithOpenAI(
      googleTranslation,
      originalText,
      targetLang
    );
